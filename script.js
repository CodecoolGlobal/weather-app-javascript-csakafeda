...
const PEXELS_API_KEY =
    "563492ad6f917000010000017dc7253158424d50865df68010506ac8";
...
const RAIN =
    "1063,1087,1150,1153,1168,1171,1183,1186,1189,1192,1195,1198,1201,1243,1246,1273,1276";
const FOG = "1030,1135,1147";
const WINTER =
    "1066,1069,1072,1114,1117,1204,1207,1210,1213,1216,1219,1222,1225,1237,1240,1249,1252,1255,1258,1261,1264,1279,1282";
const FAV_ARRAY = [];
...
const GEO_URL =
    "https://api.geoapify.com/v1/ipinfo?apiKey=1f0ce3f788934d84ada176d2cb290b2e";
...
async function searchInputCity(input) {
    ...
    const options = {
        ...
        headers: {
            "X-RapidAPI-Key":
                "3c6387f1bbmsh3257c41f0afa079p1c70e4jsn3a0f607bffe0",
...
async function getData() {
    loadData();
    await fetch(
        `http://api.weatherapi.com/v1/current.json?q=${LOCATION_NAME}&key=${WEATHER_API_KEY}`
    )
    ...
    await fetch(
        `http://api.weatherapi.com/v1/astronomy.json?q=${LOCATION_NAME}&key=${WEATHER_API_KEY}`
    )
        .then((response) => response.json())
        .then((data) => {
    ...
    await fetch(
        `http://api.weatherapi.com/v1/forecast.json?q=${LOCATION_NAME}&key=${WEATHER_API_KEY}`
    )
        .then((response) => response.json())
        .then((data) => {
...
async function onInputForAutocomplete(e) {
    let a;
    let b;
    let i;
    ...
    if (val.length >= 3) {
        ...
        for (i = 0; i < cityList.length; i++) {
            ...
            if (
                cityList[i].substr(0, val.length).toUpperCase() ==
                val.toUpperCase()
            ) {
                b = document.createElement("div");
                b.dataset.value = cityList[i];
                b.innerHTML =
                    "<strong>" +
                    cityList[i].substr(0, val.length) +
                    "</strong>";
                b.innerHTML += cityList[i].substr(val.length);
                /*b.innerHTML +=
                    '<input type="hidden" value="' + cityList[i] + '">';*/

                b.addEventListener("click", function (clickE) {
                    ...
                    if (
                        !FAV_ARRAY.includes(LOCATION_NAME.toLocaleUpperCase())
                    ) {
...
function autocomplete(CITY_INPUT) {
    ...
    CITY_INPUT.addEventListener(
        "input",
        debounce(1000, onInputForAutocomplete)
    );
...
function fillCardWithData() {
    ...
    FEELS_LIKE_ON_CARD.innerHTML =
        `Feels like: ` + FEELSLIKE + `<sup>&deg;</sup>`;
...
async function bgrndImgChange() {
    await fetch(
        `https://api.pexels.com/v1/search?query=${LOCATION_NAME}` /*${IMG_SEARCHER}**/,
        {
            headers: {
        ...
        .then((data) => {
            ...
            BGRND_IMG =
                data.photos[Math.floor(Math.random() * 10)].src.landscape;
...
function addFav() {
    ...
    let LIST_ELEMENT = document.getElementById(
